...
const CartItem = ({ itemData }) => {
  ...
  const incQty=()=>{
    Avilable_stock && Avilable_stock > Qty ? setQty(Qty + 1) : setQty(Avilable_stock);
    if(Avilable_stock>Qty){
      addToCart(itemData,undefined,{ref:'CALL_FROM_CART',plush_Or_Minus:1})
    }else(
  ...
  const decQty=()=>{
    ...
    if(Qty>1){
      addToCart(itemData,undefined,{ref:'CALL_FROM_CART',plush_Or_Minus:0})
    }else{
      ...
      <div className=" grid grid-cols-2 gap-2">
        ...
        <div className="text-xs lg:text-base md:text-base">
          ...
          <h4 className="my-1 flex justify-start items-center lg:gap-x-5 md:gap-x-5 gap-x-3 ">
            ...
            <span>{Qty}</span>
            <button className="text-lg md:text-xl lg:text-xl" onClick={incQty}><AiFillPlusCircle/></button>
          </h4>
        </div>
      </div>
    </>
